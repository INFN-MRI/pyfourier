...
def nufft_adj(
    ...
    * ``coord.shape = (nviews, nsamples, ndim) -> (1, nviews, nsamples, ndim)``
    ...
    if backend.__name__ == "torch":
        if coord is None:
            image = NUFFTAdjoint.apply(
                kspace, nufft_plan, basis, weight, threadsperblock, norm
            )
        else:
            image = NUFFTAdjointTraj.apply(
                kspace, coord, nufft_plan, basis, weight, threadsperblock, norm
            )
    else:
        image = _nufft._nufft_adj(
            kspace, nufft_plan, basis, weight, threadsperblock, norm
        )
...
if _subroutines.pytorch_enabled:
    ...
    class NUFFTAdjoint(torch.autograd.Function):
        @staticmethod
        def forward(kspace, nufft_plan, basis, weight, threadsperblock, norm):
            return _nufft._nufft_adj(
                kspace, nufft_plan, basis, weight, threadsperblock, norm
            )
    ...
    class NUFFTAdjointTraj(torch.autograd.Function):
        @staticmethod
        def forward(kspace, coord, nufft_plan, basis, weight, threadsperblock, norm):
            return _nufft._nufft_adj(
                kspace, nufft_plan, basis, weight, threadsperblock, norm
            )
        ...
        @staticmethod
        def setup_context(ctx, inputs, output):
            kspace, coord, nufft_plan, basis, weight, threadsperblock, norm = inputs
            ctx.save_for_backward(kspace, coord)
            ctx.set_materialize_grads(False)
            ctx.nufft_plan = nufft_plan
            ctx.basis = basis
            ctx.weight = weight
        ...
        @staticmethod
        def backward(ctx, image):
            kspace, coord = ctx.saved_tensors
            nufft_plan = ctx.nufft_plan
            basis = ctx.basis
            weight = ctx.weight
            threadsperblock = ctx.threadsperblock
