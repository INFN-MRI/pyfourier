...
def spiral(ndim, npix, ncontrasts, ninterleaves, R=None):
    """
    ...
    output : SimpleNamespace
        ...
        *gdt: Sampling dwell time (hardcoded to 4 us).

    """
    # rescale inputs
    fov = float(npix) * 1e-3  # mm -> m
    ...
    # initialize output
    output = SimpleNamespace(k=k, shape=[npix] * ndim, dcf=dcf, gdt=4e-6)
...
# %% local utils
def _spiral(
    fov, N, ninterleaves, R=1.0, f_sampling=1.0, alpha=1.0, gm=22, sm=250, gamma=2.678e8
):
    res = fov / N
...
def _2d_rotation(input, phi):
    coord_in = input.T
    coord_out = np.zeros((*coord_in.shape, len(phi)), dtype=coord_in.dtype)
...
def _3d_rotation(coord_in, phi):
    coord_in = input.T
    coord_out = np.zeros((3, *coord_in.shape[1:], len(phi)), dtype=coord_in.dtype)
